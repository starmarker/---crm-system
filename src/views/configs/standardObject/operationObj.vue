    ...
    <div class="layout-content">
        <div class="layout-breadcrumb">
            <p style="font-size:16px;">
                <Icon type="lock-combination"></Icon> 业务实体管理</p>
        </div>
        <div class="layout-content-main">
            <Row :gutter="24" type="flex">
                <Col span="4">
                <div class="item-block">
                    <div class="item-block-title">业务实体
                        <p class="operate" :class="{'active':iseditable}">
                           <!-- <Icon type="plus" @click.native="addDepart"></Icon>
                            ...
                            <Icon type="settings" @click.native="binding"></Icon>
                        </p>
                    </div>
...
export default {
    ...
    created() {
        ...
        this.$http.post("/web/Crm_corp_mgr_service/findCrm_corp_mgr_serviceList",{ps:10}).then((res)=>{
            if(res.data.b==1){
    ...
    methods: {
        ...
        addLayourMessagea(){
            if(this.state==0){
                    ...
                    this.$http.post("/web/Crm_corp_mgr_layout/addCrm_corp_mgr_layout",obj).then((res)=>{
                    if(res.data.b==1){
                         this.$http.post("/web/Crm_corp_mgr_layout/findCrm_corp_mgr_layoutList",{ps:10,service_id:this.service_id}).then((res)=>{
                            if(res.data.b==1){
                                  this.layoutList=res.data.a;
        ...
        getTotal(page){
            ...
            let data1={ps:10,pi:page,service_id:this.service_id};
            this.$http.post("/web/Crm_corp_mgr_service_option/findCrm_corp_mgr_service_optionList",data1).then((res)=>{
                if(res.data.b==1){
       ...
       delExtend(index){
        ...
        delLayout(index){
           ...
           this.$Modal.confirm({
                ...
                content: `是否要删除该布局`,
                onOk: () => {
                    this.$http.post("/web/Crm_corp_mgr_layout/delCrm_corp_mgr_layoutById",{layout_id:id}).then((res)=>{
                    if(res.data.b==1){
                        this.$http.post("/web/Crm_corp_mgr_layout/findCrm_corp_mgr_layoutList",{ps:10,service_id:this.service_id}).then((res)=>{
                            if(res.data.b==1){
                                  this.layoutList=res.data.a;
        ...
        //查询所有字段
        selectAllLayour(){    
            this.$http.post("/web/Crm_corp_mgr_service_option/findAllCrm_corp_mgr_service_optionList",{service_id:this.service_id}).then((res)=>{
                if(res.data.b==1){
                     res.data.a.map(obj=>{
                        obj.label=obj.ser_opt_name_show;
                        obj.key=obj.ser_opt_id;
